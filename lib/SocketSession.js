...
//                in this tate, 'disconnect' terminates session and 'connect'/'reconnect' will
//                be rejected
...
class SocketSession extends Session {
    ...
    constructor(socket, sessionId, logger, emitter) {
        ...
        // TODO : add SocketSession in models and extend it to SocketSession here
    ...
    handleNewSocket(socket, next) {
        // if session exists, (hasn't removed before) and new 'connection' has made
        //    1) client has just reconnected to server
        //    2) client reused access token in another instance
        // case 1-1
        //   - when client has disconnected, session state must have been set to LOSING
        //     if previous state was NORMAL
        //   - now client is back. we should revive the session
        //     it's not sure that socket#id is not changed
        // case 1-2
        //   - when server began protocol 'knight-fall' or just banned a session,
        //     this.state should been changed to 'CLOSING'
        //   - While this.state === 'CLOSING', 'disconnect' event will make session go away
        //   - if socket is not disconnected yet and client tries to connect with same session id,
        //     it means client just want to duplicate session socket. punish him.
        //  case 1-3
        ...
        //     knight-fall begins.
        //  case 2
        //   - each instance MUST use their own access token and session id. sharing is not allwoed
        ...
        switch(this.state) {
            ...
            case Session.STATES.CLOSING:
                if (this.closingEvent) {
                    ...
                    return next(new Error('session should have only 1 connection')); // case 2
                }
            case Session.STATES.NORMAL:
                return next(new Error('session should have only 1 connection')); // case 2
            default:
                return next(new Error('Hit the bug - session state is not valid'));
        }
    }
        ...
        socket.on('disconnect', () => {
            ...
            switch(this.state) {
                ...
                case Session.STATES.CLOSING:
                    ...
                    break;
                default:
                    err = new Error('session state is not valid');
                    this.logger.error(err, 'hit the bug');
                    break;
            }
        });
    ...
    _terminate() {
        this.logger.debug({session:this}, 'terminiting session');
        this.socket.disconnect();
        this.socket = null;
