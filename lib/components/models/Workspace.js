...
class Workspace extends AbstractDomainModel {
    ...
    // so, passing client as second arg, is ignored currently
    addClient(clientId) {
        if (!this._clients[clientId]) {
            this._clients[clientId] = true;
        }
        if (!this._watcher) {
            this._createWatcher();
            this._watcher.start();
    ...
    _publishWatcherEvent(topic, data) {
        if (this._channel) {
    ...
    _createWatcher() {
        ...
        this._watcher = new Watcher(this.id, this.workspacePath, this.apiModel.excludedPaths,
            (topic, data) => this._publishWatcherEvent(topic, data),
    ...
    createExcludedPathFilter() {
        return ignore().add(this.apiModel.excludedPaths).createFilter();
    }
    ...
    dispose() {
        ...
        this._removeWatcher();
        debug(`workspace ${this.id} is being disposed`);
    ...
    unresolvePath(localPath, absolute) {
        assert(localPath.startsWith(this.apiModel.rootPath),
            `path ${localPath} is not in workspace root path ${this.apiModel.rootPath}`);
        let vPath = localPath.slice(this.apiModel.rootPath.length + 1);
        if (path.sep === '\\') {
