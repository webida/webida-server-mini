...
class SessionRegistry extends AbstractComponent {
    ...
    constructor(server) {
        super('session-registry-registry', [], server);
        this.sessions = {};
    ...
    start() {
        return super.start().then( () => {
            ...
            this._sessionEventEmitter.on('initialized', (session) => {
                // this event will also be emitted when session-registry recovers client connection
    ...
    addSession(session) {
        this.logger.debug({session}, 'add session-registry');
