...
class Authenticator extends AbstractComponent {
    ...
    authHttpBasic(req, res, next) {
        ...
        try {
            let cred = this._parseHttpAuthorizationHeader(authHeader);
            if (cred.username !== this._server.config.loginId ||
                cred.password !== this._server.config.loginPassword) {
    ...
    _parseHttpAuthorizationHeader(headerValue) {
