...
class SocketService extends AbstractComponent {
    ...
    // topic = workspace.(un)register / workspace.watcher
    _onWorkspaceEvent(data, envelope) {
        let subject = envelope.topic.split('.')[1];
        // workspace.registered / unregistered events go to management room
        // watcher events goes to @workspace- room
        if (subject !== 'watcher') {
            this._broadcast(ROOMS.MANAGEMENT, envelope.topic, {
                workspaceId: data.id
            });
            ...
            this._broadcast(roomName, envelope.topic, data);
        }
    }

    _broadcast(roomName, topic, data) {
        if(this._io) {
            this._io.to(roomName).emit(topic, data);
            // watcher events are too verbose
            if (topic !== 'workspace.watcher') {
                this.logger.debug({
                    roomName,
                    topic,
                    data
                }, 'broadcasted socket event');
            }
        }
    }
}
