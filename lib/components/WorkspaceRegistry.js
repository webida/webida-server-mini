...
const Workspace = require('workspace/WorkspaceImpl.js');
...
class WorkspaceRegistry extends AbstractComponent {
        ...
        return super.start().then( () => {
            ...
            this._sessionEventEmitter.on('initialized', (session) => {
                ...
                if (wsid) {
                    ...
                    debug({ session }, 'workspace registry ignored non-workspace-session-registry');
                }
            });
            ...
            this._sessionEventEmitter.on('change-workspace', (oldId, session) => {
                // session-registry has always workspace id value here
    ...
    getAllWorkspaces(includeDisposable) {
        if (includeDisposable) {
            return _.pickBy(this.workspaces, ws => ws.disposable);
        } else {
            return _.values(this.workspaces);
        }
    }
