...
const wfsHandler = require('./wfs-handler.js');
...
class RestifyBuilder {
    ...
    _setPrewares() {
        ...
        function handleGitRequest(req, res, next) {
            if (!req.url.startsWith('/git/')) {
                ...
                gitHandler(myself._lookupComponent, req, res, next);
    ...
    _setMiddlewares() {
        ...
        // should not use anonymous funciton in middlware chain for audit logs( =>, too)
        let myself = this;
        function injector(req, res, next) {
            req.getServerComponent = myself._lookupComponent;
            req.getServerConfig = () => myself._config;
            next();
        }
    ...
    _setDefaultRoutes() {
        ...
        app.get('/_ping', function pingpong(req, res, next) {
            req.log.info({
                params: req.params
            }, 'got ping!');
            res.json({
        ...
        // wfs contents serrvice
        app.get('/wfs/:wfsId/:wfsPath(.*)', wfsHandler.handleDirectReadRequest);
