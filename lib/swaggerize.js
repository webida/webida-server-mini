...
var {assert, fsx, path, util, debugFactory } = global.process.libs;

var async = require('async'),
    swaggerizeRoutes = require('swaggerize-routes'),
...
//  (we already have bluebird Promise replacing native one since initialization)  
...
function makeValidator(validator, consumes) {
    ...
    function validateInput(req, res, next) {
        ...
        if (parameter.in === 'formData' && parameter.type == 'file') {
            req.log.debug( {parameter, value}, 'setting accessor for formData value');
...
function registerRoute(server, mountpath, routeSpec) {
    var path, args, before, validators, paramSpecs = {};
    ...
    if (routeSpec.security) {
        debug('add authorize function For %s', util.inspect(routeSpec.security));
        before.push(authorizeFor(routeSpec.security));
    }

    validators = [];
    for (var i = 0; i < routeSpec.validators.length; ++i) {
    ...
    debug('registering routes %s %s', routeSpec.method, util.inspect(args));
