...
const {assert, fsx, path, debugFactory, restify, thing } = __webida.libs;
...
function swaggerizeAsync(server, options) {

    assert.ok(thing.isObject(options), 'Expected options to be an object.');
    assert.ok(options.api, 'Expected an api definition.');
    
    function loadApiAsync(apiPath) {
        if (apiPath.indexOf('.yaml') === apiPath.length - 5 ||
            apiPath.indexOf('.yml') === apiPath.length - 4) {
            return fsx.readFileAsync(apiPath).then ( (data) => yaml.load(data) );
        } else {
            return fsx.readJsonAsync(apiPath);
        }
    }
    
    return loadApiAsync(options.api).then( spec => {
        options.api = spec;
...
module.exports = swaggerizeAsync;
