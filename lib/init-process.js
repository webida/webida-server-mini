...
function importProcessEnv() {
    ...
    switch (process.platform) {
        case 'win32':
            env.userHome = path.resolve(process.env.USERPROFILE);
            env.appHome = path.resolve(process.env.APPDATA, 'webida');
            break;
        case 'darwin':
            env.userHome = path.resolve(process.env.HOME);
            env.appHome = path.resolve(process.env.HOME, 'Library', 'Application Support', 'webida');
            break; 
        default:
            env.userHome = path.resolve(process.env.HOME);
            env.appHome = path.resolve(process.env.HOME, '.webida');
            break; 
    }
    ...
    // in fact, all complext options are need to be accessed in __webida.env
    _.merge(__webida.env, env, __webida.opt) ;
...
function importLibs() {
    // debugFactory will set some bluebird debugging env variables.
    // so, should be required first
